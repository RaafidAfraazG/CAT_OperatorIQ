    ...
    async function loadData() {
      try {
        ...
        if (machines.length > 0) {
          ...
          // Load intelligence/anomaly data from operator intelligence endpoint
          if (operators.length > 0) {
            const intel = await api.getOperatorIntelligence(operators[0].operator_id).catch(() => null);
            if (intel?.machine) {
              // Merge anomaly data onto machine state
              setMachine((prev: any) => ({ ...prev, ...intel.machine }));
            }
          }
        }
      } catch (err) {
